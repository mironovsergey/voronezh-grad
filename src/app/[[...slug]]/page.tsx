...
const Page = async ({ params }: { params: Promise<{ slug?: string[] }> }) => {
  ...
  try {
    data = await fetchData<UmiData>(
      `${process.env.NEXT_PUBLIC_API_URL}${path}.json`
    );
  } catch {
    notFound();
  }
  ...
  const groups = Object.values(data.page.properties.group).filter((group) =>
    group.name.endsWith('_block')
  );

  const blocks = groups
    .map((group) => {
      const activeProp = group.property[`${group.name}_active`];
      const orderProp = group.property[`${group.name}_order`];
